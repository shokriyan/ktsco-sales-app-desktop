...
public class StockService {
    ...
    public List<StockResponse> getStockReport() {
     ...
     inStock.forEach(in -> {
       BigDecimal outQuantity = outStock.stream().filter(out -> out.getProductId() == in.getProductId()).map(IStockQuantity::getQuantity).reduce(BigDecimal.ZERO, BigDecimal::add);
