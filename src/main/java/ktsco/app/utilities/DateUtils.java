...
import ktsco.app.codes.DateCode;
...
public class DateUtils {
    ...
    private static final DateTimeFormatter DEFAULT_DATE_FORMAT = DateTimeFormatter.ofPattern("yyyy-M-d");
    ...
    public static String toJalaliLocalDate(LocalDate localDate) {
        return JalaliDateUtil.gregorianToJalali(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
    }

    /**
     * Converting Jalali Date with Format yyyy-MM-dd.
     * example 1402-12-21
    ...
    private static int[] convertJalaliDate(String date) {
        try {
           ...
           if (month > 12 || day > 31 || year <= 1300) {
               throw new ErrorResponseException(HttpStatus.BAD_REQUEST, DateCode.D101.name());
           }
            return new int[]{year, month, day};
        }catch (PatternSyntaxException ex) {
            throw new ErrorResponseException(HttpStatus.BAD_REQUEST, DateCode.D100.name());
        }
    }
}
